...
############################################################################################################
AMR = c("Lowland_Andeans", "Highland_Andeans", "Maya_Chiapas",
        "PUR", "MXL", "CLM", "PEL", "Pima_Mexico", "Surui_Brazil", "Maya_Mexico",
        "Karitiana_Brazil")
high = c("Pima_Mexico", "Lowland_Andeans", "Highland_Andeans", "PEL")
...
Americas = genetocn %>%
  ...
  dplyr::mutate(
    Pop = dplyr::recode(Pop, "Highland_Andeans" = "Quechua",
                        "Lowland_Andeans" = "Quechua")) %>%
  dplyr::mutate(Pop= fct_relevel(Pop, "Surui_Brazil", "Karitiana_Brazil","Maya_Mexico",
...
geom_vline(xintercept = seq(1, 21, by = 2), linetype = "dashed") +
  ...
  xlab("") +
  ylab("") +
  scale_color_manual(values = c("Yes" = "darkgoldenrod3", "No" = "maroon4")) +
  scale_fill_manual( values = c("Yes" = "darkgoldenrod3", "No" = "maroon4")) +
  theme_bw() +
...
pdf("genetocn_amy1_AMR_rounded_histogram.pdf", height = 10, width = 9)
Americas
dev.off()
